...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    const fetchApplication = async () => {
      try {
        const res  = await fetch(`${API}/internship-application`, {
  ...
  const handleAccept = async () => {
    ...
    try {
      const res  = await fetch(`${API}/internship-application/accept`, {
        method: "POST",
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      const data = await res.json();
  ...
  const handleWithdraw = async () => {
    ...
    try {
      const res  = await fetch(`${API}/internship-application/withdraw`, {
        method: "POST",
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      const data = await res.json();
    ...
    <StudentLayout title="Dashboard">
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px" }}>
          <h3 style={{ fontSize: "18px", fontWeight: "600", color: "#1e293b", margin: 0 }}>My Internship Application Status</h3>
